<template>
    <div class="decorBlock__optionsBlock">
      <div class="decorBlock__optionItem optionItem" v-for="item in items" :key="item.ID">
        <input type="checkbox"
    ...
    export default {
      ...
      methods: {
        ...
        checkInput(section, event) {
          const { target } = event;
          this.$store.commit('changeChecked', { id: section, type: 'checkbox', checked: target.id
          });
        }
      }
    };
